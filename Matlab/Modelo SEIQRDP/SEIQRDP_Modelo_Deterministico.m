...
% * P: Inmunes. 
% 
% Dado este modelo, la tasa básica de reproducción ($R_0$) puede ser calculada 
% utilizando la expresión
% 
% 
% 
% Donde T consiste del número de días transcurridos.
...
% MatarAlpha = 1: Predicción sin inmunes (P = 0)
% MatarAlpha = 0: Predicción con inmunes
MatarAlpha = 0;
...
Po = 0;                                             % No inicial de "inmunes"
So = Poblacion - (Eo + Io + Qo + Ro + Do + Po);     % No inicial de susceptibles
CondInicial = [So Eo Io Qo Ro Do Po]';

% Array con todos los "Guesses" iniciales para todas las constantes del
% modelo a emplear.
Guess = [Alpha Beta Gamma Delta LambdaParams KappaParams];

% Se realiza la estimación de los parámetros
LimiteInf = [0 0 0 0 0 0 0 0 0];

% Cambiar límite superior si se desea "Matar a Alpha"
if MatarAlpha == 1
    LimiteSup = [0 3 1 1 1 5 100 0.009 0.1];
else
    LimiteSup = [1 3 1 1 1 5 100 0.009 0.1];
end
...
Gamma = ModelParams(3);
Delta = ModelParams(4); 
LambdaParams = ModelParams(5:7);
KappaParams = ModelParams(8:9);
...
figure(3); clf;
hold on;
semilogy(tDias, S, 'm', tDias, P, 'c');
ylabel('Número de Casos')
xlabel('Tiempo (Días)')
leg = {'Susceptibles (S)', 'Inmunes (P)'};
legend(leg{:},'location','southoutside');
title(["COVID-19: " + Pais + " (SP)"; "Max P = " + num2str(max(P))]);
set(gcf,'color','w')
...
Gamma = Params(3);
Delta = Params(4); 
LambdaParams = Params(5:7);
KappaParams = Params(8:9);
...
Poblacion = S + E + I + Q + R + D + P;

% Cálculo de las ecuaciones diferenciales
dS = -Alpha*S - Beta*(S*I/Poblacion);
